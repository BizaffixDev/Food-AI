...
class ForgotOtpVerificationScreen extends ConsumerStatefulWidget {
  String email;
  ForgotOtpVerificationScreen({super.key, required this.email});

  @override
  ConsumerState<ForgotOtpVerificationScreen> createState() => _ForgotOtpVerificationScreenState();
}

class _ForgotOtpVerificationScreenState extends ConsumerState<ForgotOtpVerificationScreen> {
  ...
  _sendOtp() {
   ...
   // context.push('${PagePath.newPassword}?email=$email}');
    ref
        .read(authNotifyProvider.notifier)
        .verifyForgetOtp(context, email: email.toString(), otp: int.parse(code));
  }
...
/*  void resendCode() async {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AuthStates>(authNotifyProvider, (previous, screenState) async {
      if (screenState is OtpVerifyForgetErrorState) {
      ...
      }  else if (screenState is OtpVerifyForgetLoadingState) {
        debugPrint('Loading');
        showLoading(context);
      } else if (screenState is OtpVerifyForgetSuccessfulState) {

        context.push('${PagePath.newPassword}?email=${widget.email}');
        dismissLoading(context);
      }
    });
